...
import { analyzePointInput, getAuthoringRules, validateAuthoringModel, validateTourRobotScene } from "./runtime.js";
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    name: "get_authoring_rules",
    description: "Load the versioned tour_robot authoring rules before generation or validation.",
    parameters: {
      category: { type: "string", enum: ["input", "model", "identity", "reference", "program", "content", "publication"], description: "Optional rule category filter." },
      enforcement: { type: "string", enum: ["schema", "validator", "agent_review", "policy"], description: "Optional enforcement filter." }
    },
    output: jsonOutput(),
    execute: getAuthoringRules
  }));
